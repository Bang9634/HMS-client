...
public class RoomManagePanel extends JPanel {
	...
	private static final String[] COLUMN_NAMES = {
		"객실번호", "기본금액", "점유상태", "최대인원", "금액변경사유", "수정", "삭제"
	};
	...
	private void updateTable(List<RoomRow> rooms) {
		...
		for (RoomRow room : rooms) {
			...
			row.add(room.priceChangeReason == null ? "" : room.priceChangeReason);
			row.add("수정");
			row.add("삭제");
	...
	private void handleDeleteRoom(int row) {
		// int roomNumber = (int) tableModel.getValueAt(row, 0);
		// int confirm = JOptionPane.showConfirmDialog(this, "정말로 객실 [" + roomNumber + "]를 삭제하시겠습니까?", "삭제 확인", JOptionPane.YES_NO_OPTION);
		// if (confirm == JOptionPane.YES_OPTION) {
		// 	ApiResponse response = roomApi.deleteRoom(new DeleteRoomRequest(roomNumber));
